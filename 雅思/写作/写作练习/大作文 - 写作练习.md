...
- 1.递进：furthermore, in addition to；
- 2.转折：however, on the other hand, let's not forget the fact that；
- 3.举例：for instance, for example, to illustrate this；
- 4.结果：consequently, as a result；
- 5.列举：firstly, next, to begin with；
- 6.原因：because of, due to, this is because；
- 7.总结：in conclusion, to sum up；
- 8.介绍：another point that I would like to add, in my opinion。
